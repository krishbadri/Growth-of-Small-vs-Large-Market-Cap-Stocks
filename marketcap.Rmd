...
bottom_10_plot <- bottom_stocks_combined |> ggplot(data=bottom_stocks_combined, mapping = aes(x = Date, y = Open, size=Volume, show.legend = FALSE)) + geom_point() + geom_smooth(se=TRUE, color="red") + scale_x_date(limits = as.Date(c("1995-01-03", "2020-04-01"))) + coord_cartesian(ylim=c(0,400)) + labs(title="Prices of Bottom 10 Weighted Stocks Since 1995", x="Year", y="Opening Price", caption="Source: Oleh Onyschak")
bottom_10_plot
```
